// File: histogram_privatized_safe.cu
// Description: A safe histogram implementation using privatization with shared memory.
#include <iostream>
#include <vector>
...
__global__ void histogram_kernel(const char* data, int* global_bins, int N) {
    ...
    __shared__ int local_data_idx;  // To coordinate which thread processes which data

    // 2. Initialize private histogram to zero - done by single thread for safety
    if (threadIdx.x == 0) {
    ...
    // 3. Each thread processes its chunk of data with proper synchronization
    ...
    __syncthreads(); // Wait for all threads to finish processing data

    // 4. Single thread per block adds private results to the global bins
    if (threadIdx.x == 0) {
        for (int i = 0; i < NUM_BINS; i++) {
