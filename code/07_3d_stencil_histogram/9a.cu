// File: histogram.cu
// Description: A complete histogram implementation using privatization with shared memory.
#include <iostream>
#include <vector>
...
__global__ void histogram_kernel(const char* data, int* global_bins, int N) {
    ...
    // 3. Each thread processes its chunk of data
...
int main() {
    ...
    std::cout << "Launching histogram kernel..." << std::endl;
