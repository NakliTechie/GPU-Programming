...
// --- Configuration ---
#define N 32 // Grid dimension (must be multiple of TILE_DIM)
#define TILE_DIM 8
#define RADIUS 1
...
__global__ void tiledStencil3D(const float* in, float* out) {
    ...
    // Only "inner" threads compute an output
        ...
        tz >= RADIUS && tz < BLOCK_DIM - RADIUS)
    {
        ...
        if (out_x < N && out_y < N && out_z < N) {
            ...
            float north  = tile[tz][ty + 1][tx];
            float south  = tile[tz][ty - 1][tx];
            float east   = tile[tz][ty][tx + 1];
            float west   = tile[tz][ty][tx - 1];
